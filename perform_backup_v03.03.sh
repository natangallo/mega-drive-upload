...
# Process directory handling both compressed and uncompressed files
process_directory() {
    local source="$1"
    local files_processed=0
    local files_skipped=0
    
    debug_log "Processing directory: '$source' (Proxmox backup mode)"
    
    # Estensione per supportare i formati di Proxmox
    local compressed_extensions=("*.tar.zst" "*.tar.gz" "*.vma.gz")
    local find_cmd_uncompressed="find \"$source\" -type f \( -not -name \"*.tar.zst\" -a -not -name \"*.tar.gz\" -a -not -name \"*.vma.gz\" \) -print0"
    local find_cmd_compressed="find \"$source\" -type f \( -name \"*.tar.zst\" -o -name \"*.tar.gz\" -o -name \"*.vma.gz\" \) -print0"

    # Processa prima i file non compressi
    while IFS= read -r -d $'\0' file; do
        debug_log "Processing uncompressed file: '$file'"
        transfer_file "$file" "compress"
        handle_transfer_result $? || break
        ((files_processed++))
    done < <(eval "$find_cmd_uncompressed")

    # Poi i file già compressi (formati Proxmox)
    while IFS= read -r -d $'\0' file; do
        debug_log "Processing Proxmox backup file: '$file'"
        transfer_file "$file" "skip_compression"
        handle_transfer_result $? || break
        ((files_processed++))
    done < <(eval "$find_cmd_compressed")

    debug_log "Processed: $files_processed files, skipped: $files_skipped"
...
transfer_file() {
    local file="$1"
    local compression_mode="${2:-compress}"
    local backup_name="backup_$(basename "$file")"
    local original_name=$(basename "$file")
    local backup_name="backup_${original_name}"
    local tmpfile="$TMP_DIR/backup_temp_${original_name}"
    
    # Clean existing backups
    clean_existing_backups "$backup_name"
    
    # Handle compression if needed
    if [[ "$compression_mode" == "compress" ]]; then
        debug_log "Compressing $file -> $tmpfile"
        if ! run_command "tar -czf \"$tmpfile\" \"$file\""; then
            error_log "Compression failed for $file"
            safe_remove "$tmpfile"
            return 1
        fi
    else
        debug_log "Using pre-compressed file: $file"
        tmpfile="$file"  # Use the file directly
    fi
    
    # Get file size
    local filesize=$(stat -c%s "$tmpfile")
    local free_space=$(get_free_space "${volumes[$curr_vol_index]}")
    
    debug_log "$(basename "$file") - Size: $(format_number $filesize) - Free: $(format_number $free_space)"
    
    # Handle transfer (standard or split)
    if (( filesize > free_space )); then
        debug_log "File too large for single volume, splitting..."
        if split_and_transfer "$file" "$tmpfile" "$filesize"; then
            [[ "$compression_mode" == "compress" ]] && safe_remove "$tmpfile"
            return 0
        else
            [[ "$compression_mode" == "compress" ]] && safe_remove "$tmpfile"
            return 1
        ...
        debug_log "Transferring '$tmpfile' to '${volumes[$curr_vol_index]}'"
        if ! run_command "mega-put '$tmpfile' '${volumes[$curr_vol_index]}/$backup_name'"; then
            error_log "Transfer failed for '$file'"
            [[ "$compression_mode" == "compress" ]] && safe_remove "$tmpfile"
            return 1
        fi
        [[ "$compression_mode" == "compress" ]] && safe_remove "$tmpfile"
...
# Handle transfer results and volume rotation
handle_transfer_result() {
    local result=$1

    case $result in
        0) debug_log "Transfer successful" ;;
        1) error_log "Transfer failed for Proxmox backup file" ;;
        2) debug_log "Volume full, rotating to next" ;;
    esac
    ...
    if (( result == 2 )); then
        ...
        if (( curr_vol_index >= ${#volumes[@]} )); then
            error_log "All volumes full during processing"
            return 1
        fi
    elif (( result != 0 )); then
        return 1
...
# Clean existing backups across all volumes
clean_existing_backups() {
    local backup_name="$1"
    debug_log "Checking for existing backups of '$backup_name'"
